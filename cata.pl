...
sub launch_game() {
   ...
   if($OPT{curses}) {
      my $command = $MAIN_CONFIG->{linux_curses_command};
      $command = "% &" unless length $command;
...
sub update_game() {
   ...
   my $executable = get_game_executable;
   #unlink $executable if -f $executable;
...
sub download_font($) {
   ...
   my $path = catfile($DOWNLOADED_FONTS, $filename);

   unless(-d $DOWNLOADED_FONTS) {
      say "Create '$DOWNLOADED_FONTS'";
      mkdir $DOWNLOADED_FONTS;
   }

   download_file($url, catfile($DOWNLOADED_FONTS, $filename));

   if($filename =~ /\.zip$/) {
      my $archive = Archive::Extract->new(archive => $path);
      $archive->extract(to => $DOWNLOADED_FONTS);
      die $archive->error if $archive->error;

      unlink $path;

      $filename = only_value { /\.ttf$/ } @{ $archive->files };
      die unless $filename;
      $path = catfile($DOWNLOADED_FONTS, $filename);

      unlink catfile($DOWNLOADED_FONTS, $_) for grep { $_ ne $filename } @{ $archive->files };
   }
...
sub install_fonts() {
   unless(-d $DOWNLOADED_FONTS) {
      die "'$DOWNLOADED_FONTS' not found. Try --fonts-download\n";
   }

   unless(-d $LINUX_INSTALLED_FONTS) {
      say "Create '$LINUX_INSTALLED_FONTS'";
      mkdir $LINUX_INSTALLED_FONTS;
   }

   say "Copy '$DOWNLOADED_FONTS' -> '$LINUX_INSTALLED_FONTS'";
   dircopy($DOWNLOADED_FONTS, $LINUX_INSTALLED_FONTS);
}
...
GetOptions \%OPT,
   ...
   # Options
   "no-download", "keep", "32bit",
   "curses", "fonts-download", "fonts-install",
